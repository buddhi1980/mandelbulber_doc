...
by repeating a formula /emph{iterating} in an iteration loop. The iterations are counted starting from Iteration 0, and at the end of each iteration the count number is increased by 1, then the next iteration of the formula commences (e.g. Iters 0, 1, 2, 3, ...). The iterating continues until \emph{termination conditions} are met, which is either when the iteration count equals the \emph{maxiter} or when the /emph({bailout}) condition is achieved.

This section explains what is calculated within the iteration loop.
...
The equations are made from mathematical operators ( +, -, \times, \div)  and can include functions (e.g.\sin, \cos, \tan, \exp, \log, \sqrt{}, pow) and also conditions (e.g. if x > y then "get the result from an equation").

A set of equations that have a specific function within the formula are called /emph{transforms}, e.g rotation, scale.

With each iteration of the formula, the point being iterated is \emph{mapped} (moved) to new coordinates.  
...
This formula is a made from a set of different transforms. It is a good example
which shows that a fractal formula can be much more complicated than the
