...
It is possible to mix different fractal formulas in a sequence within the iteration loop.
This way new fractal shapes can be achieved. These fractal combinations are named \emph{hybrid fractals}. 
There are many different fractal formulas available in the Mandelbulber program, which allows the user to create a vast variety of hybrid shapes.
...
difference is that in a hybrid iteration loop, a user can include up to nine different fractal formulas (or transforms), and create various sequences. A user can also create hybrid sequences by using other copies of a formula that have different parameter settings.
Each formula is configured in a separate slot (tab).
\simpleImageWithCaption75Width{img/manual/media/fractal_tabs.png}
{Fractal Tab - Formula only in first slot}
{fractal_tabs}

The iteration count numbers define when in the sequence each formula is calculated.
...
By default the program works in single fractal formula mode, where you can only configure the first slot, (tab #1). There are two ways to
...
Once hybrid fractals has been enabled, a user can choose additional formulas from the dropdown menus in any of the nine slots,
like as shown in figure \ref{fractal_tabs_with_defined_fractals_tabs_highlighted}.
...
one \emph{Box Fold Bulb Pow 2}. The length of the sequence is three slots, so after every third iteration the sequence repeats from the first slot. The numbers are the Iteration Count, starting at Iteration 0. The count increases by 1 after \underline{every} iteration performed in the iteration loop.
...
of the fractal has some similarities to the \emph{Mandelbulb - Power 2}.
...
With each slot a user can define how many times the fractal formula will be used in the sequence.
On each of the nine slots there is a parameter named \emph{Iterations} which is set to 1 by default.
This is the number of iterations (repeats) of the formula performed before the loop moves on to the next formula in the sequence.
If this value is increased to 2 on the first and second formula slots,
...
During running the iteration loop, the program is moving through the enabled formula slots, checking formula iteration range conditions. If the current Iteration Count number is within the range then an iteration of that formula is performed, and the Iteration Count is increased by 1.
...
because \emph{Start at iteration} was 4 for this formula and therefore the slot was skipped.
...
Because at iteration number 2 \emph{Menger Sponge} formula was skipped, the \emph{Box Fold Bulb Pow 2} formula has much more influence on the final shape.
...
Pressing these buttons causes swapping of fractal formula tabs between slots, and therefore changes the formula's position inside  the sequence.
...
Even if the same fractal formulas are used in each slot, and for the same number of iterations, the final shape will strongly depend on the parameter settings of the first few formulas that are iterated. There are also a few formulas and transforms which have a very strong influence on the final shape, and these are often run for just 1 or 2 iterations during the iterating of the fractal.
